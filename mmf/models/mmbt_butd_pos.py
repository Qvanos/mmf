...
class MMBTBUTDPOSForClassification(nn.Module):
    def __init__(self, config, *args, **kwargs):
        super().__init__()
        self.config = config
        self.bert = MMBTBase(config, *args, **kwargs)
        self.butd = BUTD(config, *args, **kwargs)
    ...
    def forward(self, sample_list):
        ...
        butd_output = self.butd(sample_list)
        captions = butd_output['captions']
...
@registry.register_model("mmbt_butd_pos")
class MMBTBUTDPOS(BaseModel):
